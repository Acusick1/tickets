...
class TicketmasterScraper(BaseScraper):
    ...
    def scrape(self, url: str) -> Dict:
        ...
        self.page.wait_for_timeout(15000)  # Wait longer for JavaScript
        ...
        try:
            ...
            # If no text prices found, try HTML content
            if '$' not in page_text and '£' not in page_text:
                logger.debug("No prices in text, trying HTML content")
                page_text = self.page.content()

            # Extract all prices from the page text using regex (support $ and £)
            # Try USD first (match prices with or without cents)
            price_matches = re.findall(
                r'\$\s*(\d+(?:,\d{3})*(?:\.\d{2})?)', page_text
            )
            currency = "USD"
            ...
            logger.error(f"Error scraping Ticketmaster: {e}", exc_info=True)
            availability = "error"
